...
process* createProcess(void* entry_point, int argc, char* argv[]) {
    ...
    new_process->rsp = createStackFrame(malloc(STACK_SIZE), entry_point, argc, argv);
...
void freeProcess(process* process) {
    free(process->rsp);  //deberia estar apuntando a stack_base no?
    free(process);
}

void* createStackFrame(void* stack_base, void* entry_point, int argc, char* argv[]) {
    stack_frame* stack_frame = ((char*)stack_base - sizeof(stack_frame));
    stack_frame->r15 = 0;
    stack_frame->r14 = 1;
    stack_frame->r13 = 2;
    stack_frame->r12 = 3;
